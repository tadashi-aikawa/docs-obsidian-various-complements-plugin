...
[[⚙️ Complement automatically]]を `OFF` にしてください。[[🎮 Show suggestions]]コマンドを実行しない限り、補完候補は表示されなくなります。
...
## コードブロックでは候補を出さないでほしい

[[⚙️ Disable suggestions in the Code block]]を有効にしてください。(インラインブロックは未対応です)
